...
from competition.libs.sign_approximators import STESignApproximator, STEWithClipSignApproximator, ParabolaSignApproximator
...
class ConvBlock(nn.Module):
    def __init__(self, n_features, group_size):
        ...
        self.bin_path = nn.Sequential(
            EConvWithLab(n_features, n_features * 2),
            Scale(n_features),
        )
        # self.bin_path = EConv(n_features, 2 * n_features)
...
class BinaryScalex2BaselineV1WithSpatial(nn.Module):
    def __init__(
        self,
        in_channels=3,
        out_channels=3,
