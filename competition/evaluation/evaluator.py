...
from competition.evaluation.checker import Checker
from competition.exceptions import NotBinaryException
from competition.libs.binary_modules import BINARY_MODULES_NAMES
from competition.tools.metrics import PSNR
...
class Evaluator:
    ...
    def evaluate_model(self, model, scale, dataset_name, device=None):
        ...
        model.eval()
        if scale in [2, 4]:
            dataloader = BenchmarkDataloader(name=dataset_name, scale=scale, num_workers=1)
        ...
        pbar = tqdm.tqdm(total = len(dataloader.loader))
        for x, y in dataloader.loader:
            ...
            pbar.update(1)
            pbar.set_postfix({'Dataset': dataset_name, 'PSNR': psnr.compute().item()})
